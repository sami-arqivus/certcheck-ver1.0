...
import { FileText, AlertCircle, Loader2, CheckCircle, XCircle, Download, X } from 'lucide-react';
...
const VerifyCard = () => {
  ...
  const downloadResult = () => {
    ...
    const filename = `verification-result-${timestamp}.txt`;
    
    const content = `CSCS Card Verification Result
Generated: ${new Date().toLocaleString()}
Task ID: ${currentTaskId || 'N/A'}

Status: ${taskResult.success ? 'SUCCESS' : 'FAILED'}
Message: ${taskResult.message || 'N/A'}

${taskResult.success && taskResult.cards_data ? 
  `Card Data:
...
---
Generated by CertCheck Admin Panel
`;
    ...
    toast({
      ...
      description: `Result saved as ${filename}`,
      variant: "default",
    });
  };
      ...
      try {
        ...
        if (status === 'SUCCESS') {
          ...
          if (result.success) {
            ...
            // Keep the result displayed - don't reset verification state
          }
          return;
                  ...
                  {taskResult.success ? (
                    <div>
                      <div className="flex items-center space-x-2 mb-3">
                        ...
                        <p className="text-green-600 font-medium">Card verification successful!</p>
                      </div>
                      {taskResult.cards_data && (
                        <div className="mt-3">
                          <h4 className="text-sm font-medium text-muted-foreground mb-2">Card Details:</h4>
                          <pre className="text-xs text-muted-foreground bg-background p-3 rounded border overflow-auto max-h-60">
                            {JSON.stringify(taskResult.cards_data, null, 2)}
                          </pre>
                        </div>
                      )}

...
const AuthForm = () => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:8080'}/forgot-password/`, {
        email,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:8080'}${endpoint}`, payload, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
      ...
      if (response.status === 200 && response.data.access_token) {
        // Handle enhanced token response
      ...
      if (error.response?.status === 429) {
        ...
        toast({
          ...
          description: `Too many attempts. Please try again in ${retryAfter || 'a few'} seconds.`,
          variant: "destructive",
        });
      } else {
        toast({
